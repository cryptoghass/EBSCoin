...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='ebs.conf'
CONFIGFOLDER='/root/.ebs'
COIN_DAEMON='ebsd'
COIN_CLI='ebs-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/Elbruscurrency/EBSCoin.git'
COIN_TGZ='https://github.com/Elbruscurrency/EBSCoin/releases/download/v1.0.3/ebs-1.0.3-x86_64-linux-gnu.tar.gz'
...
purgeOldInstallation() {
    ...
    #remove old files
    if [ -d "~/.elbrus" ]; then
        sudo rm -R ~/.elbrus> /dev/null 2>&1
    fi
    #remove binaries and elbrus utilities
    cd /usr/local/bin && sudo rm ebs-cli ebs-tx ebsd > /dev/null 2>&1 && cd
...
function important_information() {
 echo
 echo -e "${BLUE}================================================================================================================================${NC}"
 echo -e "${PURPLE}Windows Wallet Guide. https://github.com/Realbityoda/EBSCoin/blob/master/README.md${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
